...
other 234 apps — replace with real 234 Slides branding when available).

Regenerate the whole set from a new source image with:

```bash
pnpm --filter @234/slides exec tauri icon src-tauri/icons/app-icon.png
```

(`tauri icon` is a Node command — no Rust required.) A native **build**
(`pnpm --filter @234/slides tauri build --bundles nsis`) additionally needs the
